...
function Validator(formSelector) {
    ...
    var validatorRules = {
        ...
        email(value) {
            ...
            return regex.test(value) ? '' : `Não corresponde a um Email`;
        },
        min(min) {
            return (value) => {
                return value.length >= min ? '' : `Tem que possuir no minico ${min} Caractere(s)`;
            }
        },
        max(max) {
            return (value) => {
                return value.length <= max ? '' : `Maximo atingido ${max} !`;
            }
        },
        confirm(selector) {
            return (value) => {
                return value == document.querySelector(selector).value ? '' : `Lore`;
            }
        },
        checkone(name) {
            return () => {
                var checked = formElement.querySelector(`[name="${name}"]:checked`);
                return checked ? '' : `Campo obrigatorio()`;
            }
        }
